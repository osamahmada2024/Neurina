...
def pad_mirror(img, landmarks):
    ...
    img = np.pad(img, ((orig_h//2, orig_h//2), (orig_w//2, orig_w//2), (0, 0)), 'reflect')

    sigma = max(1.0, orig_h / 100.0)
    small = cv2.resize(img, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR).astype(np.float32) / 255.0
    small_blurred = cv2.GaussianBlur(small, ksize=(0, 0), sigmaX=sigma, sigmaY=sigma, borderType=cv2.BORDER_REFLECT)
    padded_h, padded_w, _ = img.shape
    blurred = cv2.resize(small_blurred, (padded_w, padded_h), interpolation=cv2.INTER_LINEAR) * 255.0

    coords = np.meshgrid(np.arange(padded_h), np.arange(padded_w), indexing="ij")
    weight_y = np.clip(coords[0] / (padded_h//4), 0, 1)
    weight_x = np.clip(coords[1] / (padded_w//4), 0, 1)
    ...
    landmarks += np.array([padded_w//4, padded_h//4])
